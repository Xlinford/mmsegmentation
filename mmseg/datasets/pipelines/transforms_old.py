...
class RandomMIOUCrop(object):
    ...
    def __init__(self, crop_size, cat_max_ratio=1., ignore_index=255, MIOU_range=(0.1, 1.0)):
        assert crop_size[0] > 0 and crop_size[1] > 0
        self.crop_size = crop_size
        self.cat_max_ratio = cat_max_ratio
        self.ignore_index = ignore_index
    ...
    def get_crop_bbox(self, img):
        ...
        crop_1 = [crop_1y1, crop_1y2, crop_1x1, crop_1x2]
        offset_corner_y = [offset_corner, offset_corner, 0, 0]
        offset_corner_x = [0, 0, offset_corner, offset_corner]
        corner_rand = [1, 1, -1, -1]
    ...
    def __call__(self, results):
        ...
        img = results['img']
        crop_bbox, crop_bbox2 = self.get_crop_bbox(img)
        ...
        for key in results.get('seg_fields', []):
            results[key] = self.crop(results[key], crop_bbox)
            results[key+'2'] = self.crop(results[key], crop_bbox2)
